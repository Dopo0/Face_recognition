...
def main():
    ...
    def is_name_taken(name):
        folder_path = os.path.join("../streamlit/data/original_faces", name)
    ...
    if uploaded_files:
        ...
        os.makedirs(f"../streamlit/data/original_faces/{user_name}", exist_ok=True)

        for file in uploaded_files:
            # Save each file to the "images" folder
            file_path = os.path.join(f"../streamlit/data/original_faces/{user_name}", file.name)

            with open(file_path, "wb") as f:
                f.write(file.read())

            st.write(f"File '{file.name}' saved successfully to 'original_faces' folder.")
